...
#set >/tmp/car.txt
echo "OPERACIONES"
echo "select active,createdby,user_name(createdby),count(*) from ingresos  group by 1,2,3 order by 2"|/usr/bin/psql --log-file=/home/nacional/demo.log -q -h 10.176.5.153 nacional megatom|grep -v rows
echo "CONTENEDORES"
echo "select active,createdby,user_name(createdby),count(*) from contenes  group by 1,2,3 order by 2"|/usr/bin/psql --log-file=/home/nacional/demo.log -q -h 10.176.5.153 nacional megatom|grep -v rows 
echo "STOCK"
